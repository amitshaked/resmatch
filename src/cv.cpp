...
int copy_make_border(lua_State *L)
{
    THFloatTensor *src_ = (THFloatTensor*)luaT_checkudata(L, 1, "torch.FloatTensor");
    THFloatTensor *dst_ = (THFloatTensor*)luaT_checkudata(L, 2, "torch.FloatTensor");

	 int top_ = luaL_checkinteger(L, 3);
	 int bottom_ = luaL_checkinteger(L, 4);
	 int left_ = luaL_checkinteger(L, 5);
	 int right_ = luaL_checkinteger(L, 6);

	 cv::Mat src_mat = cv::Mat(2, 3, CV_32FC1, src_);
	 cv::Mat dst_mat = cv::Mat(2, 3, CV_32FC1, dst_);
	 copyMakeBorder(src_mat, dst_mat, top_, bottom_, left_, right_, cv::BORDER_REFLECT_101);
